...
if 'user' not in st.session_state or st.session_state.user['id'] is None:
    ...
    with showcased_bots:
        ...
        ac.view_bot_grid(bot_dict=sb, bots_api=b, button_disabled=button_enabled)
    with my_bots:
        if len(mb) > 0:
            ...
            ac.view_bot_grid(bot_dict=mb, bots_api=b, button_disabled=button_enabled, show_bot_id=True)
        else:
            st.markdown("\n")
